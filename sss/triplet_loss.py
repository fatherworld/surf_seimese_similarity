...
class DataIter(mx.io.DataIter):
    ...
    def generate_batch(self, n):
        ...
        while len(ret) < n:
            ...
            c = []
            for i in range(32):
                a.append(d1[k1].tolist())
                b.append(d1[k2].tolist())
                c.append(d2[k3].tolist())
            a = np.array(a)
            b = np.array(b)
            c = np.array(c)
            a.reshape(32,32,3)
    ...
    def __iter__(self):
        print('begin')
        count = (int)(100000 / self.batch_size)
        for i in range(count):
            ...
            data_all = [mx.nd.array(batch_same).reshape(128,3,32,32), mx.nd.array(batch_diff).reshape(128,3,32,32), \
                        mx.nd.array(batch_one).reshape(128)]
            label_all = [mx.nd.array(batch_anchor).reshape(128,3,32,32)]
